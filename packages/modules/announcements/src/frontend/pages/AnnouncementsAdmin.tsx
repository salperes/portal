...
import { CategoryLabels, PriorityLabels, PriorityColors } from '@portal/core';

// API functions type - injected from main app
interface AnnouncementsAdminApi {
  getAll: (includeInactive?: boolean) => Promise<Announcement[]>;
...
interface AnnouncementsAdminPageProps {
  api: AnnouncementsAdminApi;
}
...
export function AnnouncementsAdminPage({ api }: AnnouncementsAdminPageProps) {
  ...
  const { data: announcements, isLoading } = useQuery({
    ...
    queryFn: () => api.getAll(true),
  });

  const createMutation = useMutation({
    mutationFn: (data: CreateAnnouncementDto) => api.create(data),
    onSuccess: () => {
              ...
              <tbody className="divide-y divide-gray-100">
                {announcements?.map((ann) => (
                  <tr key={ann.id} className="hover:bg-[#f3f2f1] transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`px-2 py-0.5 text-xs font-medium ${PriorityColors[ann.priority].bg} ${PriorityColors[ann.priority].text}`}
                      >
