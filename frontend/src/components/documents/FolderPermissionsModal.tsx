...
import { X, Loader2, Shield, Trash2, UserPlus, Users, FolderOpen, Briefcase } from 'lucide-react';
import { documentsApi } from '../../services/documentsApi';
import type { FolderPermissionRule } from '../../services/documentsApi';
...
export function FolderPermissionsModal({ folderId, folderName, onClose }: FolderPermissionsModalProps) {
  ...
  const { data: rules = [], isLoading: rulesLoading } = useQuery({
    ...
    queryFn: () => documentsApi.getFolderPermissions(folderId),
  });
  ...
  const addMutation = useMutation({
    mutationFn: (data: { targetType: 'user' | 'group'; targetId: string; permissions: string[] }) =>
      documentsApi.addFolderPermission(folderId, data),
    onSuccess: () => {
  ...
  const removeMutation = useMutation({
    mutationFn: (ruleId: string) => documentsApi.removeFolderPermission(folderId, ruleId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['folder-permissions', folderId] });
    },
  });
    ...
    if (activeTab === 'project' && (targetId === '__all__' || !targetId) && projectGroups.length > 0) {
      ...
      try {
        for (const g of projectGroups) {
          await documentsApi.addFolderPermission(folderId, {
            targetType: 'group',
            targetId: g.id,
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-2xl max-h-[85vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-500 dark:text-gray-400">-- {folderName}</span>
          </div>
          ...
          <div>
            ...
            {rulesLoading ? (
              ...
              <div className="space-y-2">
                {rules.map((rule: FolderPermissionRule) => (
                  <div
                    key={rule.id}
